...
function setDbConnString() {
  ...
  if (config.get('env') === 'test') config.set('dbConnectionString.connString', `mongodb://${URL}?${QUERY}`);
  else config.set('dbConnectionString.connString', `mongodb://${DBAUTH}@${URL}?${QUERY}`);

  return config;
}
const config = setDbConnString();
